...
class P2pChat(tk.Frame):
    def __init__(self, master=None):
        ...
        master.wm_title("Chat Application")
    ...
    def createWidgets(self):
        ...
        menu.add_command(label="Show Connections",command=self.destroy)
        menu.add_separator()
        ...
        msg_window = scrolledtext.ScrolledText(msg_frame, height=20, width=60)
        ...
        # msg entry frame to specific user
        msg_entry_frame_usr = tk.Frame(msg_frame, relief=tk.RAISED, bd=1)
        msg_entry_frame_usr.pack(side=tk.BOTTOM, fill=tk.X)

        msg_entry_usr = tk.Entry(msg_entry_frame_usr)
        msg_entry_usr.pack(side=tk.LEFT, fill=tk.X, expand=1)
        msg_entry_usr.focus()
        self.msg_entry_usr = msg_entry_usr
        
        send_btn_usr = tk.Button(msg_entry_frame_usr)
        send_btn_usr["text"] = "SendToUser"
        send_btn_usr["command"] = self.send_msg
        send_btn_usr.pack(side=tk.RIGHT)
